...
import {
  MessageCircle, Send, Upload, Users, Calendar, Save, X,
  CheckCircle, AlertCircle, Clock, FileText, Smartphone, Settings,
  Pause, Play, Edit, Trash2, Filter, ChevronDown, ChevronUp, Plus,
  BarChart3, Zap, Activity, Globe, Copy, Eye, Music, Image as ImageIcon, Video
} from 'lucide-react';
...
const generateMockCampaigns = (count: number) => {
  ...
  const types = ['text', 'image', 'video', 'sticker', 'template'];
...
const mockTemplates = [
  { id: 'lpl_1', name: 'Product Carousel', body_content: 'Check out our new arrivals!', category: 'Marketing' },
  { id: 'lpl_2', name: 'Order Status', body_content: 'Your package is on the way.', category: 'Transactional' },
  { id: 'lpl_3', name: 'Support Ticket', body_content: 'How would you rate our service?', category: 'Survey' }
];

export default function BulkLine() {
  const { user } = useAuth();
  ...
  const [deliverySpeed, setDeliverySpeed] = useState(30); // msgs/sec
    ...
    <div className="min-h-screen bg-[#012419] p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-slate-400">Manage broadcasts and engage with your LINE audience</p>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="template">Rich Message (Template)</option>
                    </select>
                  </div>
                </div>

                <div>
                   <label className="block text-sm font-medium text-slate-300 mb-2">User IDs (One per line)</label>
                   <textarea
                      value={pastedIds}
      ...
      {activeTab === 'advanced' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                <div>
                 ...
                 <div className="flex items-center gap-3">
                   ...
                   <span className="text-sm text-slate-300">Disable Push Notifications (Silent Message)</span>
                 </div>
              </div>
            </div>
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
              <h3 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
                <Settings className="w-5 h-5 text-[#39FF14]" /> Rich Menus
              </h3>
              <div className="space-y-4">
                 <label className="block text-sm font-medium text-slate-300">Default Rich Menu ID</label>
                 <input
                    type="text"
                    value={richMenuId}
